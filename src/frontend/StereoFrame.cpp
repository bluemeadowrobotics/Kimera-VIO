...
void StereoFrame::undistortRectifyPoints(
  ...
  for (const KeypointCV& px : left_keypoints_unrectified) {
    ...
    bool cropped = UtilsOpenCV::cropToSize(
        &px_undistRect, cam_param.undistort_rectify_map_x_.size());
    ...
    if (!FLAGS_images_rectified) {
      ...
      x_check = cam_param.undistort_rectify_map_x_.at<float>(
          round(px_undistRect.y), round(px_undistRect.x));
      y_check = cam_param.undistort_rectify_map_y_.at<float>(
          round(px_undistRect.y), round(px_undistRect.x));
    } else {
      x_check = px.x;
      y_check = px.y;
    ...
    if (cropped || std::fabs(px.x - x_check) > kTolInPixels ||
      ...
      VLOG(10) << "undistortRectifyPoints: pixel mismatch\n"
               ...
               << cam_param.undistort_rectify_map_x_.size() << '\n'
               << "map type " << cam_param.undistort_rectify_map_x_.type()
               << '\n'
...
cv::Mat StereoFrame::getDisparityImage() const {
  ...
  if (is_debug) {
    ...
    disparity_img.convertTo(
        img_disparity_8U, CV_8UC1, 255 / (max_val - min_val));
...
void StereoFrame::computeRectificationParameters(
  ...
  if (left_camera_info.distortion_model_ == DistortionModel::RADTAN) {
    // Get stereo rectification
    VLOG(10) << "Stereo camera distortion for rectification: radtan";
    cv::stereoRectify(
        // Input
        left_camera_info.K_,
        left_camera_info.distortion_coeff_mat_,
        right_camera_info.K_,
        right_camera_info.distortion_coeff_mat_,
        ...
        P2,
        Q);
  } else if (left_camera_info.distortion_model_ ==
  ...
  *B_Pose_camRrect =
  ...
  if (right_camera_info.distortion_model_ == DistortionModel::RADTAN) {
    ...
    cv::initUndistortRectifyMap(left_camera_info.K_,
                                ...
                                CV_32FC1,
                                // output:
                                left_camera_info.undistort_rectify_map_x_,
                                left_camera_info.undistort_rectify_map_y_);
  } else if (right_camera_info.distortion_model_ ==
             DistortionModel::EQUIDISTANT) {
  ...
  if (right_camera_info.distortion_model_ == DistortionModel::RADTAN) {
    ...
    cv::initUndistortRectifyMap(right_camera_info.K_,
                                ...
                                CV_32FC1,
                                // Output:
                                right_camera_info.undistort_rectify_map_x_,
                                right_camera_info.undistort_rectify_map_y_);
  } else if (right_camera_info.distortion_model_ ==
             DistortionModel::EQUIDISTANT) {
    // Get rectification & undistortion maps. (equi dist. model)
